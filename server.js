...
app.get("/inputtime/:myroom", function (request, response) {
    ...
    /*Times[myroom][name_count] = time;
    Times[myroom] = Times[myroom].slice(0, name_count + 1);


    if (Times[myroom].length > name_count)

        Times[myroom].slice(0, name_count);


    //Times[myroom].push = time;
    //console.log(times[myroom]);
    console.log(Times[myroom]);
    */
...
app.all("/gettime/:myroom", function (request, response) {
    ...
    var myroom = Number(request.params.myroom); //방번호를 받아온다
    ...
    response.send(JSON.stringify(countNames));//countNnames 목록을 넘겨준다
